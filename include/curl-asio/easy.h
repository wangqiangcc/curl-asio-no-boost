...
#define IMPLEMENT_CURL_OPTION(FUNCTION_NAME, OPTION_NAME, OPTION_TYPE) \
	...
	{ \
		...
		asio::detail::throw_error(ec, STRINGIZE(FUNCTION_NAME)); \
	} \
	inline void FUNCTION_NAME(OPTION_TYPE arg, asio::error_code& ec) \
	{ \
		ec = asio::error_code(native::curl_easy_setopt(handle_, OPTION_NAME, arg), asio::system_category()); \
	}
...
#define IMPLEMENT_CURL_OPTION_BOOLEAN(FUNCTION_NAME, OPTION_NAME) \
	...
	inline void FUNCTION_NAME(bool enabled, asio::error_code& ec) \
	{ \
		ec = asio::error_code(native::curl_easy_setopt(handle_, OPTION_NAME, enabled ? 1L : 0L), asio::system_category()); \
	}
...
#define IMPLEMENT_CURL_OPTION_ENUM(FUNCTION_NAME, OPTION_NAME, ENUM_TYPE, OPTION_TYPE) \
	...
	inline void FUNCTION_NAME(ENUM_TYPE arg, asio::error_code& ec) \
	{ \
		ec = asio::error_code(native::curl_easy_setopt(handle_, OPTION_NAME, (OPTION_TYPE)arg), asio::system_category()); \
	}
...
#define IMPLEMENT_CURL_OPTION_STRING(FUNCTION_NAME, OPTION_NAME) \
	...
	inline void FUNCTION_NAME(const char* str, asio::error_code& ec) \
	{ \
		ec = asio::error_code(native::curl_easy_setopt(handle_, OPTION_NAME, str), asio::system_category()); \
	} \
	...
	inline void FUNCTION_NAME(const std::string& str, asio::error_code& ec) \
	{ \
		ec = asio::error_code(native::curl_easy_setopt(handle_, OPTION_NAME, str.c_str()), asio::system_category()); \
	}
...
#define IMPLEMENT_CURL_OPTION_GET_STRING(FUNCTION_NAME, OPTION_NAME) \
	...
	{ \
		char *info = NULL; \
		asio::error_code ec = asio::error_code(native::curl_easy_getinfo(handle_, OPTION_NAME, &info), asio::system_category()); \
		asio::detail::throw_error(ec, STRINGIZE(FUNCTION_NAME)); \
		return info; \
	}
...
namespace curl
{
	...
	{
	public:
		typedef std::function<void(const asio::error_code& err)> handler_type;
		...
		inline void set_http_auth(httpauth_t auth, bool auth_only, asio::error_code& ec)
		{
			long l = ((long)auth | (auth_only ? CURLAUTH_ONLY : 0L));
			ec = asio::error_code(native::curl_easy_setopt(handle_, native::CURLOPT_HTTPAUTH, l), asio::system_category());
		}
		...
		void set_post_fields(const std::string& post_fields, asio::error_code& ec);
		...
		void set_http_post(std::shared_ptr<form> form, asio::error_code& ec);
		...
		void add_header(const std::string& name, const std::string& value, asio::error_code& ec);
		void add_header(const std::string& header);
		void add_header(const std::string& header, asio::error_code& ec);
		void set_headers(std::shared_ptr<string_list> headers);
		void set_headers(std::shared_ptr<string_list> headers, asio::error_code& ec);
		void add_http200_alias(const std::string& http200_alias);
		void add_http200_alias(const std::string& http200_alias, asio::error_code& ec);
		void set_http200_aliases(std::shared_ptr<string_list> http200_aliases);
		void set_http200_aliases(std::shared_ptr<string_list> http200_aliases, asio::error_code& ec);
		...
		void add_mail_rcpt(const std::string& mail_rcpt, asio::error_code& ec);
		void set_mail_rcpts(std::shared_ptr<string_list> mail_rcpts);
		void set_mail_rcpts(std::shared_ptr<string_list> mail_rcpts, asio::error_code& ec);
		...
		void add_quote(const std::string& quote, asio::error_code& ec);
		void set_quotes(std::shared_ptr<string_list> quotes);
		void set_quotes(std::shared_ptr<string_list> quotes, asio::error_code& ec);
		...
		void add_resolve(const std::string& resolved_host, asio::error_code& ec);
		void set_resolves(std::shared_ptr<string_list> resolved_hosts);
		void set_resolves(std::shared_ptr<string_list> resolved_hosts, asio::error_code& ec);
		...
		inline void set_ssl_verify_host(bool verify_host, asio::error_code& ec)
		{
			ec = asio::error_code(curl_easy_setopt(handle_, native::CURLOPT_SSL_VERIFYHOST, verify_host ? 2L : 0L), asio::system_category());
		}
		...
		void set_share(std::shared_ptr<share> share, asio::error_code& ec);
		...
		void add_telnet_option(const std::string& option, const std::string& value, asio::error_code& ec);
		void add_telnet_option(const std::string& telnet_option);
		void add_telnet_option(const std::string& telnet_option, asio::error_code& ec);
		void set_telnet_options(std::shared_ptr<string_list> telnet_options);
		void set_telnet_options(std::shared_ptr<string_list> telnet_options, asio::error_code& ec);
		...
		void handle_completion(const asio::error_code& err);
